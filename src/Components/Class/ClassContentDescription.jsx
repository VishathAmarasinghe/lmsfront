...
const ClassContentDescription = ({ classDetails }) => {
  ...
  const setDescritiondetail = (classDetails) => {
    const items = [
      {
        label: 'Class ID',
      ...
      {
        label: 'Class Time',
        children: `${String(classDetails.StartTime).slice(0, -2)} - ${String(classDetails.endTime).slice(0, -2)}  `,
        span: 1,
      },
      {
        label: 'Grade',
  ...
  useEffect(() => {
    const itemsarray = setDescritiondetail(classDetails);
    setDescriptionList(itemsarray);
  }, [classDetails]);
