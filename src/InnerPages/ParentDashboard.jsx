...
const ParentDashboard = () => {
  ...
  const fetchStudentCalender = async () => {
    try {
        const childResult = await getAllchildsOfParent(activeUser?.UserID);
        console.log("children Result ", childResult);
        const classCalenderData = [];

        for (const child of childResult.data) {
            const classCalenderResult = await getClassesByStudentCalender(child.UserID);
            classCalenderData.push(...classCalenderResult.data); 
            console.log("class calendar result ", classCalenderResult);
        }

       
        setClassCount(classCalenderData.length);
        const nextDaysClassArrangement = getNextNDaysClasses(classCalenderData, 7);
